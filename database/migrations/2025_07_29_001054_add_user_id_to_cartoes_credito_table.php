...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (!Schema::hasColumn('cartoes_credito', 'user_id')) {
            Schema::table('cartoes_credito', function (Blueprint $table) {
                $table->unsignedBigInteger('user_id')->nullable()->after('id');
            });
        }

        // Encontra o primeiro usuário (admin) e atualiza os cartões existentes
        $firstUser = User::orderBy('id', 'asc')->first();
        if ($firstUser) {
            DB::table('cartoes_credito')->whereNull('user_id')->update(['user_id' => $firstUser->id]);
        }

        // Torna a coluna obrigatória e adiciona a chave estrangeira
        Schema::table('cartoes_credito', function (Blueprint $table) {
            if (DB::table('cartoes_credito')->whereNull('user_id')->doesntExist()) {
