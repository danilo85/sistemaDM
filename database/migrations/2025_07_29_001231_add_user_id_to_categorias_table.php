...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // Encontra o primeiro usuário (admin) e atualiza as categorias existentes
        $firstUser = User::orderBy('id', 'asc')->first();
        if ($firstUser) {
            DB::table('categorias')->whereNull('user_id')->update(['user_id' => $firstUser->id]);
        }

        // Torna a coluna obrigatória e adiciona a chave estrangeira
        Schema::table('categorias', function (Blueprint $table) {
            if (DB::table('categorias')->whereNull('user_id')->doesntExist()) {
