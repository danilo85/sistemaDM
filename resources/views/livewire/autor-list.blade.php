        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @forelse ($autores as $autor)
                    <tr wire:key="{{ $autor->id }}" class="border-b bg-white dark:border-gray-700 dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td class="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                            <a href="{{ route('autores.show', $autor) }}" class="flex items-center gap-2 hover:underline">
                        ...
                        <td class="px-6 py-4">
                            @if($autor->email)
                                <a href="mailto:{{ $autor->email }}" class="text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:underline">
                                    {{ $autor->email }}
                                </a>
                            @else
                        ...
                        <td class="px-6 py-4">
                            @if($autor->contact)
                                <a href="https://wa.me/{{ preg_replace('/\D/', '', $autor->contact) }}" target="_blank" class="text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:underline">
                                    {{ $autor->contact }}
                                </a>
                            @else
                                <span class="text-gray-400 dark:text-gray-500">-</span>
                        ...
                        <td class="px-6 py-4 text-right">
                           <div class="flex items-center justify-end gap-4">
                                <a href="{{ route('autores.edit', $autor->id) }}" class="text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-500" title="Editar">
                                ...
                                {{-- CORREÇÃO: O botão agora chama o método deleteAutor e usa wire:confirm --}}
                                <button 
                                    wire:click="deleteAutor({{ $autor->id }})" 
