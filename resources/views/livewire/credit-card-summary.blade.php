{{-- Card de Cartão de Crédito com Expansão --}}
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4 flex-grow cursor-pointer" @click="expanded = !expanded">
                @if($fatura->cartao->bandeira_path)
                    <img src="{{ $fatura->cartao->bandeira_path }}" alt="Bandeira" class="h-10 w-16 object-contain rounded-md bg-gray-50 dark:bg-gray-700 p-1">
                @else
                ...
                <div class="flex-1 min-w-0">
                    <div class="font-bold text-lg text-white whitespace-nowrap overflow-hidden text-ellipsis">{{ $fatura->cartao->nome }}</div>
                    <div class="text-sm text-white/80">{{ count($fatura->transacoes) }} {{ count($fatura->transacoes) == 1 ? 'despesa' : 'despesas' }} na fatura</div>
            ...
            <div class="flex items-center gap-3">
                <div class="text-right">
                    <div class="font-mono text-2xl font-bold text-white">- R$ {{ number_format($fatura->total, 2, ',', '.') }}</div>
                ...
                <button wire:click="pagarFatura" 
                        class="px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 transform hover:scale-105 shadow-md
                               {{ !$faturaEstaPaga ? 'bg-white text-red-600 hover:bg-red-50' : 'bg-white/20 text-white hover:bg-white/30' }}">
                                ...
                                <div class="flex items-center gap-3">
                                    <div class="flex-shrink-0">
                                        @if($transacao->categoria)
                                            <div class="w-3 h-3 rounded-full" style="background-color: {{ $transacao->categoria->cor ?? '#6B7280' }}"></div>
                                        @else
                                            <div class="w-3 h-3 rounded-full bg-gray-400"></div>
                                        @endif
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-medium text-gray-900 dark:text-gray-100 whitespace-nowrap overflow-hidden text-ellipsis">{{ $transacao->descricao }}</p>
