<tr @dblclick="toggleSelection({{ $orcamento->id }}, '{{ $orcamento->valor_total }}', $event)"
    :class="{ '!bg-blue-100 dark:!bg-blue-900/50': selectedIds.includes({{ $orcamento->id }}) }" 
    ...
    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
        <a href="{{ route('orcamentos.show', $orcamento->id) }}" class="hover:underline">
            <span class="font-bold">#{{ $orcamento->numero }}</span> - {{ $orcamento->cliente->name ?? 'N/A' }}
        </a>
    </td>
    {{-- Coluna Título --}}
    <td class="px-6 py-4">
    ...
    <td class="px-6 py-4">
        <div x-data="{ 
            ...
            {{-- O menu dropdown que abre e fecha, agora com x-teleport e posicionamento dinâmico --}}
    ...
    <td class="px-6 py-4 text-right text-lg font-semibold text-green-600">
        <span class="whitespace-nowrap">R$ {{ number_format($orcamento->valor_total, 2, ',', '.') }}</span>
    </td>
    {{-- Coluna Data --}}
    <td class="px-6 py-4">
    ...
    {{-- Coluna de Ações com Ícones Diretos --}}
    <td class="px-6 py-4 text-right">
        <div class="flex items-center justify-end gap-4">
            ...
            <a href="{{ route('orcamentos.edit', $orcamento->id) }}" class="text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-500" title="Editar">
