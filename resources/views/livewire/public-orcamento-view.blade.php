<div>
    {{-- Importação da fonte Open Sans e estilo específico para esta página --}}
    ...
    <div class="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 font-open-sans">
        <div class="bg-white p-8 md:p-12 shadow-lg rounded-md relative print-container">
            ...
            @endphp
            <div title="Status: {{ $orcamento->status }}" class="no-print absolute top-8 right-8 z-10 flex h-6 w-6 items-center justify-center">
                <span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-opacity-75 {{ $statusClass }}"></span>
                <span class="relative inline-flex h-4 w-4 rounded-full {{ $statusClass }}"></span>
            </div>
            ...
            <header class="flex justify-between items-start mb-12">
                <div>
                    ...
                    <div class="mt-4 text-sm text-gray-500">
                        <p>Válido de {{ $orcamento->data_emissao->format('d/m/Y') }} a {{ $orcamento->data_validade->format('d/m/Y') }}</p>
                        <p>Para <span class="font-semibold text-gray-700">{{ $orcamento->cliente->name }}</span></p>
                    </div>
                </div>
                <div>
                    <div class="bg-gray-800 text-white w-20 h-20 flex items-center justify-center rounded-full">
                        <span class="text-2xl font-bold">{{ $orcamento->numero }}</span>
                    </div>
                </div>
            </header>
            ...
            <main>
                ...
                <div class="mb-10">
                    <p class="font-bold text-gray-800">Prazo: <span class="font-normal text-gray-600">Aproximadamente {{ $orcamento->prazo_entrega_dias }} dias úteis.</span></p>
                </div>

                <div class="bg-gray-100 rounded-md p-8 mb-10 print-bg-transparent">
                    <div>
                        <p class="text-gray-500 uppercase text-sm">Total</p>
                        <p class="text-6xl font-extrabold text-gray-800">R$ {{ number_format($orcamento->valor_total, 2, ',', '.') }}</p>
                        <p class="mt-2 text-sm text-gray-500">Forma de pagamento: {{ $orcamento->condicoes_pagamento }}</p>
                    </div>
                </div>

                @if($parcelas == 2 && $orcamento->valor_total > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 text-white font-bold rounded-md overflow-hidden print-grid-cols-2">
                        <div class="bg-gray-800 p-8">
                            ...
                            <p class="text-3xl font-bold">1º R$ {{ number_format($orcamento->valor_total * 0.4, 2, ',', '.') }}</p>
                        </div>
                        <div class="bg-gray-700 p-8">
                            <p class="text-sm uppercase">60% ao término</p>
                            <p class="text-3xl font-bold">2º R$ {{ number_format($orcamento->valor_total * 0.6, 2, ',', '.') }}</p>
                        </div>
                    </div>
                @endif
                ...
                <div class="bg-white p-6 mt-10" x-data="{ confirmingApproval: false, confirmingRejection: false }">
                    ...
                    @else
                        <div class="text-center">
                            <p class="text-lg font-semibold italic text-gray-600">Este orçamento foi {{ strtolower($orcamento->status) }} em {{ $orcamento->updated_at->format('d/m/Y') }}.</p>
            ...
            <footer class="mt-12 flex justify-between items-center no-print">
                <div class="flex items-center gap-6">
                    <div>
                        @if(optional($orcamento->user)->logo_path)
                            <img src="{{ asset('storage/' . $orcamento->user->logo_path) }}" alt="Logo" class="h-16">
                        @else
                            <p class="font-bold text-lg">{{ $orcamento->user->name }}</p>
                        @endif
                    </div>
                    <div class="text-sm text-gray-500 space-y-2">
                        @if(optional($orcamento->user)->website_url)
                            <a href="{{ $orcamento->user->website_url }}" target="_blank" class="flex items-center gap-2 hover:text-blue-500">
                                ...
                                <span>{{ $orcamento->user->website_url }}</span>
                            </a>
                        @endif
                        @if(optional($orcamento->user)->behance_url)
                            <a href="{{ $orcamento->user->behance_url }}" target="_blank" class="flex items-center gap-2 hover:text-blue-500">
                                ...
                                <span>{{ $orcamento->user->behance_url }}</span>
                            </a>
                        @endif
                         @if(optional($orcamento->user)->whatsapp)
                            <a href="https://wa.me/{{ preg_replace('/\D/', '', $orcamento->user->whatsapp) }}" target="_blank" class="flex items-center gap-2 hover:text-blue-500">
                                ...
                                <span>{{ $orcamento->user->whatsapp }}</span>
                            </a>
                        @endif
                    </div>
                </div>
