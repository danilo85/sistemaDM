...
{{-- Card de Transação Remodelado --}}
<div class="relative overflow-hidden rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 {{ $transacao->tipo === 'receita' ? 'bg-gradient-to-br from-green-500 via-green-600 to-green-700' : 'bg-gradient-to-br from-red-500 via-red-600 to-red-700' }}">
    {{-- Efeito de brilho --}}
    ...
    <div class="relative p-6 text-white">
        ...
        <div class="flex sm:hidden flex-col items-center text-center gap-3 mb-4">
            {{-- Logo --}}
            @if($transacao->transacionavel_type === 'App\\Models\\Banco')
                @if($transacao->transacionavel && $transacao->transacionavel->logo_path)
                    <div class="w-14 h-14 bg-white rounded-full p-2 flex items-center justify-center flex-shrink-0">
                        <img src="{{ $transacao->transacionavel->logo_path }}" alt="Logo" class="w-10 h-10 object-contain">
                    </div>
                @else
                ...
                @endif
            @else
                @if($transacao->transacionavel && $transacao->transacionavel->bandeira_path)
                    <div class="w-14 h-14 bg-white rounded-full p-2 flex items-center justify-center flex-shrink-0">
                        <img src="{{ $transacao->transacionavel->bandeira_path }}" alt="Bandeira" class="h-8 w-12 object-contain">
                    </div>
                @else
                    <div class="w-14 h-14 bg-white rounded-full flex items-center justify-center flex-shrink-0">
                        <svg class="w-7 h-7 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="flex flex-wrap items-center gap-2">
            @unless($isDetalheFatura)
                <button wire:click="togglePago" 
                        class="{{ $transacao->pago ? 'bg-orange-500/80 hover:bg-orange-400/80' : 'bg-white/20 hover:bg-white/30' }} backdrop-blur-sm text-white font-semibold rounded-lg px-4 py-2 text-sm flex items-center gap-2 transition-all duration-200 transform hover:scale-105"
                        title="{{ $transacao->pago ? 'Marcar como Pendente' : 'Marcar como Pago' }}">
                    @if($transacao->pago)
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <a href="{{ route('transacoes.edit', $transacao->id) }}" 
               class="bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white font-semibold rounded-lg px-4 py-2 text-sm flex items-center gap-2 transition-all duration-200 transform hover:scale-105"
               title="Editar">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="text-center">
                ...
                <div class="mb-6">
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        ...
                        @if($transacao->compra_id && $transacao->parcela_total > 1)
                            <br><br>
