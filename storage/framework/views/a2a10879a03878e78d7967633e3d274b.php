...
>
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-4">
            <div class="p-4 bg-blue-50 dark:bg-blue-900/50 rounded-lg text-center">
                ...
                <span class="text-2xl font-bold text-blue-600 dark:text-blue-400 whitespace-nowrap">R$ <?php echo e(number_format($totalAReceber, 2, ',', '.')); ?></span>
            </div>
    ...
    <div class="mb-4 flex flex-wrap items-center gap-2">
        ...
        <button wire:click="setFiltroStatus('ativos')" class="px-3 py-1 text-sm font-semibold rounded-full transition-colors <?php echo e($filtroStatus === 'ativos' ? $activeStatusColors['ativos'] : $statusColors['ativos']); ?>">
            Ativos
        </button>
        <button wire:click="setFiltroStatus('')" class="px-3 py-1 text-sm font-semibold rounded-full transition-colors <?php echo e($filtroStatus === '' ? $activeStatusColors['todos'] : $statusColors['todos']); ?>">
            Todos
        </button>
        <div class="h-4 border-l border-gray-300 dark:border-gray-600 mx-2"></div>
        <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $statusDisponiveis; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <button wire:click="setFiltroStatus('<?php echo e($status); ?>')" class="px-3 py-1 text-sm font-semibold rounded-full transition-colors <?php echo e($filtroStatus === $status ? ($activeStatusColors[$status] ?? '') : ($statusColors[$status] ?? '')); ?>">
                <?php echo e($status); ?>
                ...
                <tr>
                    <th scope="col" class="px-6 py-3">
                        <button wire:click="sortBy('cliente_name')" class="flex items-center gap-1 uppercase">
                            Cliente
                            <!--[if BLOCK]><![endif]--><?php if($sortField === 'cliente_name'): ?>
                    ...
                    <th scope="col" class="px-6 py-3 text-right">
                        <button wire:click="sortBy('valor_total')" class="flex items-center gap-1 ml-auto uppercase">
                            Valor
                            <!--[if BLOCK]><![endif]--><?php if($sortField === 'valor_total'): ?>
                                <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="<?php echo e($sortDirection === 'asc' ? 'm5 15l7-7 7 7' : 'm19 9l-7 7-7-7'); ?>" /></svg>
                            <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                        </button>
                    </th>
...
[$__name, $__params] = $__split('orcamento-row', ['orcamento' => $orcamento]);

$__html = app('livewire')->mount($__name, $__params, $orcamento->id, $__slots ?? [], get_defined_vars());
...
<?php /**PATH C:\laragon\www\sistemaDM\resources\views/livewire/orcamento-list.blade.php ENDPATH**/ ?>