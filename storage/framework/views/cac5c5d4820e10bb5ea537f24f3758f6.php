...
<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
    <td class="px-6 py-4"><?php echo e(\Carbon\Carbon::parse($transacao->data)->format('d/m/Y')); ?></td>
    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
        <?php echo e($transacao->descricao); ?>

        
        
        <!--[if BLOCK]><![endif]--><?php if($transacao->parcela_total): ?>
        ...
        <?php elseif($transacao->is_recurring): ?>
    ...
    <td class="px-6 py-4"><?php echo e($transacao->categoria->nome ?? 'Sem Categoria'); ?></td>
    <td class="px-6 py-4 text-right text-lg font-semibold <?php echo e($transacao->tipo === 'receita' ? 'text-green-500' : 'text-red-500'); ?>">
    ...
    <td class="px-6 py-4 text-center">
        <!--[if BLOCK]><![endif]--><?php if(!$isDetalheFatura): ?>
            <button wire:click="togglePago" class="px-3 py-1 text-xs font-bold rounded-full <?php echo e($transacao->pago ? 'bg-green-200 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-gray-200 text-gray-800 dark:bg-gray-600 dark:text-gray-300'); ?>">
                <?php echo e($transacao->pago ? ($transacao->tipo === 'receita' ? 'Recebido' : 'Pago') : ($transacao->tipo === 'receita' ? 'Receber' : 'Pagar')); ?>

            </button>
        <?php else: ?>
            <span class="px-2 py-1 text-xs font-semibold rounded-full <?php echo e($transacao->pago ? 'bg-green-200 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-gray-200 text-gray-800 dark:bg-gray-600 dark:text-gray-300'); ?>">
                <?php echo e($transacao->pago ? 'Pago' : 'Pendente'); ?>
            ...
            <template x-teleport="body">
                <div x-show="confirmingDelete" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" style="display: none;">
                    <div @click.away="confirmingDelete = false" class="w-full max-w-md p-6 bg-white dark:bg-gray-800 rounded-lg shadow-xl">
                        ...
                        <p class="mt-2 text-sm text-center text-gray-600 dark:text-gray-400"><!--[if BLOCK]><![endif]--><?php if(!empty($transacao->compra_id)): ?> Esta é uma transação parcelada. Como deseja prosseguir? <?php else: ?> Tem certeza que deseja excluir este lançamento? <?php endif; ?><!--[if ENDBLOCK]><![endif]--></p>
                        <!--[if BLOCK]><![endif]--><?php if(!empty($transacao->compra_id)): ?>
                            <div class="mt-6 space-y-3">
                                ...
                                <button wire:click="deleteSingle" @click="confirmingDelete = false" class="w-full justify-center text-gray-900 bg-yellow-400 hover:bg-yellow-500 font-medium rounded-lg text-sm px-5 py-2.5">Excluir Apenas Esta (<?php echo e($transacao->parcela_atual); ?>/<?php echo e($transacao->parcela_total); ?>)</button>
